...
public class NewBehaviorScript : MonoBehaviour
{
    ...
    void OnGUI()
    {
        GUI.Box(new Rect(400, 200, 200, 300), "");
        GUIStyle style = new GUIStyle();
        style.normal.textColor = new Color(1, 1, 1);
        style.fontSize = 25;

        if (GUI.Button(new Rect(430, 290, 100, 50), "reset"))
        {
            Reset();
        }
        if (turn == 0)
        {
            GUI.Label(new Rect(260, 220, 100, 50), "○ turn:", style);
        }
        else
            GUI.Label(new Rect(260, 220, 100, 50), "× turn:", style);
        ...
        if (winner == 1)
        {
            GUI.Label(new Rect(430, 300, 100, 50), "○ win!!!", style);
        }
        else if (winner == 2)
        {
            GUI.Label(new Rect(430, 300, 100, 50), "× win!!!", style);
        ...
        if (flag == 0)//平局
        {
            GUI.Label(new Rect(430, 300, 100, 50), "A dead heat! Please restart!", style);
        }
        ...
        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                if (map[i, j] == 1)
                {
                    GUI.Button(new Rect(i * 50 + 410, j * 50 + 200, 50, 50), "○");
                }
                if (map[i, j] == 2)
                {
                    GUI.Button(new Rect(i * 50 + 410, j * 50 + 200, 50, 50), "×");
                }
                if (GUI.Button(new Rect(i * 50 + 410, j * 50 + 200, 50, 50), ""))
                {
                    if (turn == 0) map[i, j] = 1;
                    if (turn == 1) map[i, j] = 2;
                    turn = 1 - turn;
